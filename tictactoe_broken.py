...
def add_piece(game, player, row, column):
    ...
    """
    game[row][column] = player #removing +1 for proper function working
    return game
...
if __name__ == '__main__':
    ...
    while winner == 0 and moves_exist(game):
        ...
        available = False
        while not available: #fixed by adding collen 
            row = convert_input_to_coordinate(int(input("Which row? (start with 1) ")))
            column = convert_input_to_coordinate(int(input("Which column? (start with 1) ")))
            available = check_space_empty(game, row,column) #added column argument
        game = add_piece(game, player, row, column)
        display_game(game)
        player = switch_player(player)
        #removed the commented line so winner function runs
